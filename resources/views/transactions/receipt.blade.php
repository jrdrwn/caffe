        ...
        <div class="receipt-header">
            <div class="receipt-logo">
                @if($transaction->cafe && $transaction->cafe->logo_url)
                    <img src="{{ asset('storage/' . $transaction->cafe->logo_url) }}" alt="Logo Cafe" style="max-height: 56px; max-width: 100%; border-radius: 12px;">
                @else
            ...
            @php
                $locationParts = array_filter([
                    $transaction->cafe->address ?? null,
                    $transaction->cafe->city ?? null,
                    $transaction->cafe->province ?? null,
                ]);
            @endphp
