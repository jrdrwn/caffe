<x-filament-panels::page>
        ...
        <div class="pos-layout">
            ...
            <section class="pos-products">
                <div class="pos-products-header">
                    <div class="pos-products-title">
                        <span class="pos-products-icon">
                            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M18 8h1a4 4 0 0 1 0 8h-1"/><path d="M2 8h16v9a4 4 0 0 1-4 4H6a4 4 0 0 1-4-4V8z"/><line x1="6" y1="1" x2="6" y2="4"/><line x1="10" y1="1" x2="10" y2="4"/><line x1="14" y1="1" x2="14" y2="4"/></svg>
                        </span>
                        <div>
                            <h3>Menu Produk</h3>
                ...
                <div class="pos-grid">
                    @forelse($products as $product)
                        ...
                        <article
                            class="pos-card {{ $isOutOfStock ? 'is-out' : '' }}"
                        ...
                        >
                            <div class="pos-image-wrapper">
                                ...
                                @else
                                    <div class="pos-image-fallback">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1" opacity="0.3"><path d="M18 8h1a4 4 0 0 1 0 8h-1"/><path d="M2 8h16v9a4 4 0 0 1-4 4H6a4 4 0 0 1-4-4V8z"/><line x1="6" y1="1" x2="6" y2="4"/><line x1="10" y1="1" x2="10" y2="4"/><line x1="14" y1="1" x2="14" y2="4"/></svg>
                                    </div>
                                @endif
                                @if($isOutOfStock)
                            ...
                            >{{ ($product['has_variants'] ?? false) ? 'Pilih Opsi' : ($isOutOfStock ? 'Stok Habis' : '+ Tambah') }}</button>
                        </article>
                    @empty
                        <div class="pos-empty-state">
                            <svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1" opacity="0.3" style="margin-bottom:.5rem"><rect x="2" y="7" width="20" height="14" rx="2"/><path d="M16 21V5a2 2 0 0 0-2-2h-4a2 2 0 0 0-2 2v16"/></svg>
            ...
            <aside class="pos-sidebar">
                ...
                <div class="pos-panel" id="payment-panel" style="display:none">
                    <div class="pos-panel-header">
                        <div class="pos-panel-title">
                            <svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="1" y="4" width="22" height="16" rx="2" ry="2"/><line x1="1" y1="10" x2="23" y2="10"/></svg>
                            Pembayaran
                        </div>
                    </div>
                    <div class="pos-payment-body">
                        ...
                        <div class="pos-field-row">
                            ...
                            <input type="number" id="paid-amount" min="0" placeholder="0" class="pos-input pos-input-lg">
                        </div>
    ...
    <div id="variant-modal" class="pos-modal hidden" role="dialog" aria-modal="true">
        <div class="variant-container">
            ...
            <div class="variant-footer">
                <button type="button" id="variant-confirm" class="pos-btn-checkout" style="margin:0;width:100%">Tambah ke Keranjang</button>
            </div>
        </div>
    </div>
    ...
    <style>
        * { box-sizing: border-box; }
        ...
        /* ── Variant Modal ── */
        .variant-container { background:rgb(255 255 255); color:rgb(var(--color-gray-900,17 24 39)); border-radius:16px; box-shadow:0 24px 64px rgba(0,0,0,.35); width:min(400px,94vw); overflow:hidden; }
        .dark .variant-container { background:rgb(var(--color-gray-800,31 41 55)); color:rgb(var(--color-gray-100,243 244 246)); }
        .variant-header { display:flex; align-items:center; justify-content:space-between; padding:1.25rem 1.5rem; border-bottom:1px solid rgb(var(--color-gray-200,229 231 235)); }
        .dark .variant-header { border-color:rgb(var(--color-gray-700,55 65 81)); }
        .variant-header h3 { margin:0; font-size:1rem; font-weight:700; }
        .variant-close-btn { background:none; border:none; font-size:1.5rem; cursor:pointer; color:rgb(var(--color-gray-500,107 114 128)); line-height:1; padding:.25rem; }
        .variant-body { padding:1.25rem 1.5rem; display:flex; flex-direction:column; gap:1rem; }
        .variant-group-label { font-size:.72rem; font-weight:700; text-transform:uppercase; letter-spacing:.05em; color:rgb(var(--color-gray-500,107 114 128)); margin-bottom:.5rem; }
        .variant-options { display:flex; flex-wrap:wrap; gap:.5rem; }
        .variant-opt { padding:.45rem 1rem; border-radius:9999px; border:1.5px solid rgb(var(--color-gray-300,209 213 219)); background:transparent; color:rgb(var(--color-gray-700,55 65 81)); font-size:.85rem; font-weight:500; cursor:pointer; transition:all .15s; }
        .dark .variant-opt { border-color:rgb(var(--color-gray-600,75 85 99)); color:rgb(var(--color-gray-300,209 213 219)); }
        .variant-opt:hover { border-color:rgb(var(--color-primary-400,251 191 36)); }
        .variant-opt.selected { border-color:rgb(var(--color-primary-600,217 119 6)); background:rgb(var(--color-primary-50,255 251 235)); color:rgb(var(--color-primary-700,180 83 9)); font-weight:600; }
        .dark .variant-opt.selected { background:rgb(var(--color-primary-950,28 25 23)); color:rgb(var(--color-primary-300,252 211 77)); }
        .variant-footer { padding:1rem 1.5rem 1.5rem; }

        /*  Toast  */
        .pos-toast { position:fixed; top:1.25rem; right:1.25rem; z-index:9999; padding:.75rem 1.25rem; border-radius:8px; font-size:.875rem; font-weight:500; box-shadow:0 4px 16px rgba(0,0,0,.2); }
        .pos-toast-hidden { display:none; }
        .pos-toast-error   { background:rgb(var(--color-danger-600,220 38 38));  color:rgb(255 255 255); }
        .pos-toast-info    { background:rgb(var(--color-gray-700,55 65 81));     color:rgb(255 255 255); }
        .pos-toast-success { background:rgb(var(--color-success-600,22 163 74)); color:rgb(255 255 255); }

        /*  Receipt Modal  */
        .receipt-modal { position:fixed; inset:0; background:rgba(0,0,0,.65); display:flex; align-items:center; justify-content:center; z-index:2000; backdrop-filter:blur(4px); }
        ...
        .receipt-btn-outline { background:transparent; border:1.5px solid rgb(var(--color-gray-300,209 213 219)); color:rgb(var(--color-gray-700,55 65 81)); }
        .dark .receipt-btn-outline { border-color:rgb(var(--color-gray-600,75 85 99)); color:rgb(var(--color-gray-300,209 213 219)); }
        .receipt-btn-outline:hover { background:rgb(var(--color-gray-100,243 244 246)); }
        .dark .receipt-btn-outline:hover { background:rgb(var(--color-gray-700,55 65 81)); }
        .receipt-btn-primary { background:rgb(var(--color-primary-600,217 119 6)); border:none; color:rgb(var(--color-white,255 255 255)); }
        .receipt-btn-primary:hover { background:rgb(var(--color-primary-700,180 83 9)); }

        /*  Layout  */
        .pos-app { width:100%; }
        .pos-layout { display:grid; grid-template-columns:minmax(0,2fr) 340px; gap:1.25rem; align-items:start; }
        @media (max-width:1100px) { .pos-layout { grid-template-columns:1fr; } }

        /*  Products Section  */
        .pos-products { background:rgb(255 255 255); border:1px solid rgb(var(--color-gray-200,229 231 235)); border-radius:12px; overflow:hidden; }
        .dark .pos-products { background:rgb(var(--color-gray-900,17 24 39)); border-color:rgb(var(--color-gray-700,55 65 81)); }

        .pos-products-header { display:flex; align-items:center; justify-content:space-between; padding:1rem 1.25rem; border-bottom:1px solid rgb(var(--color-gray-200,229 231 235)); background:rgb(var(--color-gray-50,249 250 251)); }
        .dark .pos-products-header { border-color:rgb(var(--color-gray-700,55 65 81)); background:rgb(var(--color-gray-800,31 41 55)); }
        .pos-products-title { display:flex; align-items:center; gap:.75rem; }
        .pos-products-icon { font-size:1.5rem; }
        .pos-products-title h3 { margin:0; font-size:.95rem; font-weight:700; }
        .pos-products-title p { margin:0; font-size:.75rem; color:rgb(var(--color-gray-500,107 114 128)); }

        /*  Category Tabs  */
        .pos-category-tabs { display:flex; gap:.5rem; padding:.75rem 1rem; overflow-x:auto; border-bottom:1px solid rgb(var(--color-gray-200,229 231 235)); scrollbar-width:none; }
        .dark .pos-category-tabs { border-color:rgb(var(--color-gray-700,55 65 81)); }
        .pos-category-tabs::-webkit-scrollbar { display:none; }
        .pos-cat-btn { padding:.35rem .875rem; border-radius:9999px; border:1.5px solid rgb(var(--color-gray-200,229 231 235)); background:transparent; color:rgb(var(--color-gray-600,75 85 99)); font-size:.8rem; font-weight:500; cursor:pointer; white-space:nowrap; transition:all .15s; }
        .dark .pos-cat-btn { border-color:rgb(var(--color-gray-700,55 65 81)); color:rgb(var(--color-gray-400,156 163 175)); }
        .pos-cat-btn:hover { border-color:rgb(var(--color-primary-400,251 191 36)); color:rgb(var(--color-primary-700,180 83 9)); }
        .pos-cat-btn.active { background:rgb(var(--color-primary-600,217 119 6)); border-color:rgb(var(--color-primary-600,217 119 6)); color:rgb(255 255 255); }

        /*  Product Grid  */
        .pos-grid { display:grid; grid-template-columns:repeat(auto-fill,minmax(155px,1fr)); gap:.875rem; padding:1rem; max-height:calc(100vh - 280px); overflow-y:auto; }
        .pos-card { border:1px solid rgb(var(--color-gray-200,229 231 235)); border-radius:10px; overflow:hidden; display:flex; flex-direction:column; background:rgb(255 255 255); transition:all .15s ease; }
        .dark .pos-card { border-color:rgb(var(--color-gray-700,55 65 81)); background:rgb(var(--color-gray-800,31 41 55)); }
        .pos-card:hover:not(.is-out) { border-color:rgb(var(--color-primary-400,251 191 36)); box-shadow:0 4px 16px rgb(var(--color-primary-400,251 191 36) / 0.15); transform:translateY(-2px); }
        .pos-card.is-out { opacity:.5; }
        .pos-image-wrapper { position:relative; width:100%; height:100px; background:rgb(var(--color-gray-100,243 244 246)); overflow:hidden; }
        .dark .pos-image-wrapper { background:rgb(var(--color-gray-700,55 65 81)); }
        .pos-image { width:100%; height:100%; object-fit:cover; }
        .pos-image-fallback { width:100%; height:100%; display:flex; align-items:center; justify-content:center; font-size:2rem; }
        .pos-out-badge { position:absolute; top:.4rem; right:.4rem; background:rgb(var(--color-danger-600,220 38 38)); color:rgb(255 255 255); font-size:.65rem; font-weight:700; padding:.15rem .45rem; border-radius:4px; }
        .pos-card-body { padding:.65rem .75rem; flex:1; display:flex; flex-direction:column; gap:.2rem; }
        .pos-card-body h4 { margin:0; font-size:.85rem; font-weight:600; line-height:1.3; }
        .pos-price { margin:0; font-size:.9rem; font-weight:700; color:rgb(var(--color-primary-600,217 119 6)); }
        .pos-stock { margin:0; font-size:.7rem; color:rgb(var(--color-gray-500,107 114 128)); }
        .pos-stock-empty { color:rgb(var(--color-danger-600,220 38 38)); font-weight:600; }
        .pos-add-btn { border:0; border-top:1px solid rgb(var(--color-gray-100,243 244 246)); padding:.55rem; font-size:.8rem; font-weight:600; background:rgb(var(--color-primary-50,255 251 235)); color:rgb(var(--color-primary-700,180 83 9)); cursor:pointer; transition:background .15s; }
        .dark .pos-add-btn { border-color:rgb(var(--color-gray-700,55 65 81)); background:rgb(var(--color-primary-950,28 25 23)); color:rgb(var(--color-primary-300,252 211 77)); }
        .pos-add-btn:hover:not(:disabled) { background:rgb(var(--color-primary-100,254 243 199)); }
        .pos-add-btn:disabled { cursor:not-allowed; opacity:.5; }
        .pos-empty-state { grid-column:1/-1; text-align:center; padding:3rem 1rem; color:rgb(var(--color-gray-400,156 163 175)); }
        .pos-empty-icon { font-size:2.5rem; margin-bottom:.5rem; }

        /*  Sidebar  */
        .pos-sidebar { display:flex; flex-direction:column; gap:.875rem; position:sticky; top:1rem; }

        /*  Panels  */
        .pos-panel { background:rgb(255 255 255); border:1px solid rgb(var(--color-gray-200,229 231 235)); border-radius:12px; overflow:hidden; }
        .dark .pos-panel { background:rgb(var(--color-gray-900,17 24 39)); border-color:rgb(var(--color-gray-700,55 65 81)); }
        .pos-panel-header { display:flex; align-items:center; justify-content:space-between; padding:.75rem 1rem; background:rgb(var(--color-gray-50,249 250 251)); border-bottom:1px solid rgb(var(--color-gray-200,229 231 235)); }
        .dark .pos-panel-header { background:rgb(var(--color-gray-800,31 41 55)); border-color:rgb(var(--color-gray-700,55 65 81)); }
        .pos-panel-title { display:flex; align-items:center; gap:.4rem; font-size:.875rem; font-weight:700; }
        .pos-badge { background:rgb(var(--color-primary-100,254 243 199)); color:rgb(var(--color-primary-700,180 83 9)); padding:.15rem .55rem; border-radius:9999px; font-size:.75rem; font-weight:600; }
        .dark .pos-badge { background:rgb(var(--color-primary-950,28 25 23)); color:rgb(var(--color-primary-300,252 211 77)); }

        /*  Cart  */
        .pos-cart-list { max-height:220px; overflow-y:auto; padding:.5rem .75rem; display:flex; flex-direction:column; gap:.4rem; }
        .pos-cart-empty { text-align:center; padding:1.5rem; color:rgb(var(--color-gray-400,156 163 175)); font-size:.85rem; }
        .pos-cart-row { display:flex; align-items:center; gap:.5rem; background:rgb(var(--color-gray-50,249 250 251)); border:1px solid rgb(var(--color-gray-200,229 231 235)); border-radius:8px; padding:.55rem .7rem; }
        .dark .pos-cart-row { background:rgb(var(--color-gray-800,31 41 55)); border-color:rgb(var(--color-gray-700,55 65 81)); }
        .pos-cart-row-info { flex:1; min-width:0; }
        .pos-cart-row-name { font-size:.82rem; font-weight:600; }
        .pos-cart-row-price { font-size:.72rem; color:rgb(var(--color-gray-500,107 114 128)); }
        .pos-cart-row-controls { display:flex; gap:.2rem; align-items:center; }
        .qty-btn { width:22px; height:22px; border:1px solid rgb(var(--color-gray-300,209 213 219)); border-radius:4px; background:rgb(255 255 255); color:rgb(var(--color-gray-700,55 65 81)); cursor:pointer; font-weight:700; font-size:.8rem; display:flex; align-items:center; justify-content:center; transition:all .1s; }
        .dark .qty-btn { background:rgb(var(--color-gray-700,55 65 81)); border-color:rgb(var(--color-gray-600,75 85 99)); color:rgb(var(--color-gray-200,229 231 235)); }
        .qty-btn:hover { background:rgb(var(--color-gray-100,243 244 246)); }
        .qty-btn-remove { border-color:rgb(var(--color-danger-200,254 202 202)); color:rgb(var(--color-danger-600,220 38 38)); }
        .qty-btn-remove:hover { background:rgb(var(--color-danger-50,254 242 242)); }
        .qty-display { width:24px; text-align:center; font-size:.82rem; font-weight:700; }
        .pos-cart-row-value { font-size:.82rem; font-weight:700; color:rgb(var(--color-primary-600,217 119 6)); min-width:65px; text-align:right; }

        /*  Summary / Pricing  */
        .pos-pricing-body { padding:.875rem 1rem; display:flex; flex-direction:column; gap:.6rem; }
        ...
        .pos-input-lg { padding:.65rem .75rem; font-size:1rem; font-weight:600; }
        .pos-field-result { font-size:.78rem; color:rgb(var(--color-gray-500,107 114 128)); min-width:70px; text-align:right; }
        .pos-summary-block { background:rgb(var(--color-gray-50,249 250 251)); border-radius:8px; padding:.65rem .875rem; display:flex; flex-direction:column; gap:.35rem; margin-top:.25rem; }
        .dark .pos-summary-block { background:rgb(var(--color-gray-800,31 41 55)); }
        .pos-summary-row { display:flex; justify-content:space-between; font-size:.82rem; color:rgb(var(--color-gray-600,75 85 99)); }
        .dark .pos-summary-row { color:rgb(var(--color-gray-400,156 163 175)); }
        .pos-total-row { display:flex; justify-content:space-between; font-size:1rem; font-weight:700; padding-top:.5rem; border-top:1.5px solid rgb(var(--color-gray-200,229 231 235)); margin-top:.25rem; }
        .dark .pos-total-row { border-color:rgb(var(--color-gray-700,55 65 81)); color:rgb(var(--color-primary-300,252 211 77)); }

        /*  Payment  */
        .pos-payment-body { padding:.875rem 1rem; display:flex; flex-direction:column; gap:.875rem; }
        .pos-method-group { display:grid; grid-template-columns:1fr 1fr 1fr; gap:.4rem; }
        .pos-method-btn { padding:.55rem .35rem; border:1.5px solid rgb(var(--color-gray-200,229 231 235)); border-radius:8px; background:rgb(255 255 255); color:rgb(var(--color-gray-600,75 85 99)); font-size:.78rem; font-weight:600; cursor:pointer; transition:all .15s; }
        .dark .pos-method-btn { background:rgb(var(--color-gray-800,31 41 55)); border-color:rgb(var(--color-gray-700,55 65 81)); color:rgb(var(--color-gray-300,209 213 219)); }
        .pos-method-btn.active { border-color:rgb(var(--color-primary-500,245 158 11)); background:rgb(var(--color-primary-50,255 251 235)); color:rgb(var(--color-primary-700,180 83 9)); }
        .dark .pos-method-btn.active { background:rgb(var(--color-primary-950,28 25 23)); color:rgb(var(--color-primary-300,252 211 77)); }
        .pos-change-box { display:flex; justify-content:space-between; align-items:center; background:rgb(var(--color-success-50,240 253 244)); border:1px solid rgb(var(--color-success-200,187 247 208)); border-radius:8px; padding:.75rem 1rem; }
        .dark .pos-change-box { background:rgb(var(--color-success-950,5 46 22)); border-color:rgb(var(--color-success-800,22 101 52)); }
        .pos-change-box span { font-size:.82rem; color:rgb(var(--color-success-700,21 128 61)); }
        .dark .pos-change-box span { color:rgb(var(--color-success-400,74 222 128)); }
        .pos-change-box strong { font-size:1rem; font-weight:700; color:rgb(var(--color-success-700,21 128 61)); }
        .dark .pos-change-box strong { color:rgb(var(--color-success-400,74 222 128)); }
        .pos-btn-checkout { width:100%; padding:.8rem; border:none; border-radius:8px; background:rgb(var(--color-success-600,22 163 74)); color:rgb(255 255 255); font-size:.9rem; font-weight:700; cursor:pointer; transition:all .15s; }
        .pos-btn-checkout:hover:not(:disabled) { background:rgb(var(--color-success-700,21 128 61)); }
        .pos-btn-checkout:disabled { opacity:.4; cursor:not-allowed; }
        .pos-btn-cancel { width:100%; padding:.65rem; border:1px solid rgb(var(--color-gray-200,229 231 235)); border-radius:8px; background:transparent; color:rgb(var(--color-gray-500,107 114 128)); font-size:.85rem; cursor:pointer; transition:all .15s; }
        .dark .pos-btn-cancel { border-color:rgb(var(--color-gray-700,55 65 81)); color:rgb(var(--color-gray-400,156 163 175)); }
        .pos-btn-cancel:hover { background:rgb(var(--color-gray-100,243 244 246)); }
        .dark .pos-btn-cancel:hover { background:rgb(var(--color-gray-800,31 41 55)); }
        ...
        @media print {
            ...
            .receipt-header h2 { color:rgb(0 0 0) !important; font-size:12pt !important; letter-spacing:.08em !important; }
            .receipt-logo    { font-size:1.5rem !important; }
            .receipt-trx-num { color:rgb(80 80 80) !important; font-size:8pt !important; }
            .receipt-body    { padding:.4rem .75rem !important; gap:.3rem !important; }
            .receipt-section-label { color:rgb(60 60 60) !important; font-size:7pt !important; margin:.25rem 0 .1rem !important; }
            .receipt-item,
            .receipt-row     { font-size:9pt !important; }
            .receipt-divider { border-color:rgb(0 0 0) !important; margin:.25rem 0 !important; }
            .receipt-total-row { border-color:rgb(0 0 0) !important; color:rgb(0 0 0) !important; font-size:11pt !important; padding:.4rem 0 !important; }
            .text-success    { color:rgb(0 128 60) !important; }
            .receipt-footer  { padding:.25rem 0 !important; }
            .receipt-time    { color:rgb(100 100 100) !important; font-size:8pt !important; }
            .receipt-thanks  { color:rgb(0 128 60) !important; font-size:9pt !important; }
            .receipt-actions { display:none !important; visibility:hidden !important; }
        }
    </style>
<script>
    (() => {
